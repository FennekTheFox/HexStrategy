...
bool SPaperFlipbookWidget::SetMaterial(UMaterialInstanceDynamic* InMaterial)
{
	if (MID != InMaterial)
	{
		MID = InMaterial;
...
void SPaperFlipbookWidget::CalculateCurrentFrame()
{
	...
	if (CachedFrameIndex != LastCachedFrame)
	{
		if (UPaperSprite* Sprite = GetSpriteAtCachedIndex())
		{
			...
			if (MID)
			{
				MID->SetTextureParameterValue("SourceTexture", Sprite->GetBakedTexture());

				FAdditionalSpriteTextureArray AdditionalTextures;
				Sprite->GetBakedAdditionalSourceTextures(AdditionalTextures);

				FVector2D StartUV = SpriteAtlasData.StartUV;
				FVector2D SizeUV = SpriteAtlasData.SizeUV;
				MID->SetVectorParameterValue(TEXT("UVData"), FLinearColor(StartUV.X, StartUV.Y, SizeUV.X, SizeUV.Y));

				for (int i = 0; i < AdditionalTextures.Num(); i++)
				{
					FString ParamName = FString::Printf(TEXT("AdditionalSourceTexture_%d"), i);
					MID->SetTextureParameterValue(FName(ParamName), AdditionalTextures[i]);
				}
				MID->SetTextureParameterValue("SourceTexture", Sprite->GetBakedTexture());
				DefaultBrush.SetResourceObject(MID);
			}
			else
			{
