...
void UPaperFlipbookUserWidget::SynchronizeProperties()
{
	...
	{
		if ((Material && MID && MID->Parent != Material) || (Material && !MID))
		{
...
void UPaperFlipbookUserWidget::SetMaterial(UMaterialInterface* InMaterial)
{
	Material = InMaterial;
	if (MyFlipbookWG.IsValid())
	{
		if (Material && MID->Parent != Material)
		{
			MID = UMaterialInstanceDynamic::Create(Material, this);
		}
		else if (!Material)
